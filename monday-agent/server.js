...
const server = http.createServer((req, res) => {
    ...
    if (req.url === '/intake' && req.method === 'POST') {
        ...
        req.on('end', async () => {
            ...
            try {
                // Parseer de binnenkomende JSON
                const payload = JSON.parse(body);

                console.log(`\n🚀 Nieuwe intake ontvangen — sessie: ${payload.sessionId}`);

                // Verwerk de intake en sla op in Monday
                const resultaat = await verwerkIntake(payload);

                // Stuur succesmelding terug naar Agent 1
                res.writeHead(200, { 'Content-Type': 'application/json' });
